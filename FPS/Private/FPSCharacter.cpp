...
void AFPSCharacter::EquipWeapon(AFPSLauncher * launcher)
{
	FPSWeaponLauncher = launcher;
	FPSWeaponLauncher->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));
	FPSWeaponLauncher->SetActorHiddenInGame(false);
...
void AFPSCharacter::Exit()
{
	...
	if (FPSWeaponLauncher) {
		FP_Gun->SetHiddenInGame(false);
		FPSWeaponLauncher->Exit();
	}
}
